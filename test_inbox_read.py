...
def test_inbox_read_returns_all_messages(tmp_path, monkeypatch):
    ...
    op_inbox_append(from_="builder", to="planner", kind="note", body="b",
                    proposal=None, in_reply_to=None, verdict=None, for_version=None)
    msgs = op_inbox_read(role=None, unacked=False, since=None, proposal=None)
...
def test_inbox_read_filters_by_role(tmp_path, monkeypatch):
    ...
    op_inbox_append(from_="planner", to="human", kind="note", body="for-human",
                    proposal=None, in_reply_to=None, verdict=None, for_version=None)
    builder_msgs = op_inbox_read(role="builder", unacked=False, since=None, proposal=None)
...
def test_inbox_read_filters_by_proposal(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CONDUCTOR_DIR", str(tmp_path))
    op_inbox_append(from_="planner", to="builder", kind="note", body="a",
                    proposal="P-001", in_reply_to=None, verdict=None, for_version=None)
    op_inbox_append(from_="planner", to="builder", kind="note", body="b",
                    proposal="P-002", in_reply_to=None, verdict=None, for_version=None)
    msgs = op_inbox_read(role=None, unacked=False, since=None, proposal="P-001")
    assert [m["body"] for m in msgs] == ["a"]


def test_inbox_read_unacked_only_excludes_acked_messages(tmp_path, monkeypatch):
    """`unacked=True` returns messages addressed to the role that lack a
    ...
    op_inbox_append(
        ...
        proposal=None, in_reply_to=m1, verdict=None, for_version=None,
    )
    m2 = op_inbox_append(from_="planner", to="builder", kind="note", body="b",
                         proposal=None, in_reply_to=None, verdict=None, for_version=None)
    msgs = op_inbox_read(role="builder", unacked=True, since=None, proposal=None)
    assert [m["id"] for m in msgs] == [m2]


def test_inbox_read_unacked_only_excludes_own_messages(tmp_path, monkeypatch):
    """`unacked=True --role X` should not return X's own messages, even
    ...
    msgs = op_inbox_read(role="planner", unacked=True, since=None, proposal=None)
    assert [m["id"] for m in msgs] == [foreign]


def test_inbox_read_without_unacked_still_returns_own_messages(tmp_path, monkeypatch):
    """The self-filter only applies when `unacked=True`. A normal
    ...
    own = op_inbox_append(
        from_="planner", to="both", kind="note",
        body="audit", proposal="P-001", in_reply_to=None, verdict=None, for_version=None,
    )
    msgs = op_inbox_read(role="planner", unacked=False, since=None, proposal=None)
